...
export default function Home() {
  ...
  const fetchSessions = useCallback(async () => {
    try { const r = await axios.get("http://localhost:5000/sessions"); setSessions(r.data); }
    catch (e) { console.warn("Sessions unavailable", e); }
  }, []);
  ...
  const loadSession = async (sid) => {
    try {
      setIsLoading(true);
      const r = await axios.get(`http://localhost:5000/history?session_id=${sid}`);
      setSessionId(sid);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await axios.delete(`http://localhost:5000/sessions/${sessionToDelete}`);
  ...
  const handleFeedback = async (query, type, idx) => {
    try {
      await axios.post("http://localhost:5000/feedback", { message: query, session_id: sessionId, feedback: type });
  ...
  const exportChat = () => {
    ...
    const a = Object.assign(document.createElement("a"), { href: URL.createObjectURL(blob), download: `nexus_${Date.now()}.txt` });
  ...
  const sendMessage = async (e, override) => {
    ...
    setIsLoading(true);
    try {
      const res = await axios.post("http://localhost:5000/chat", { message: text, session_id: sessionId });
      setMessages((p) => [...p, { role: "bot", text: res.data.response, id: Date.now() + 1, isNew: true, userQuery: text }]);
      fetchSessions();
    } catch {
      setMessages((p) => [...p, { role: "bot", text: "⚠️ Could not reach the server. Please try again.", id: Date.now() + 1, isNew: true }]);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const ftr = isDark ? "bg-slate-950/80 border-slate-800/30" : "bg-white/80 border-slate-200/50";
  const bot = isDark ? "glass-panel text-slate-200 rounded-tl-none" : "bg-white border border-slate-200 shadow-xl shadow-slate-200/30 rounded-tl-none";
  ...
  return (
    <div className={`flex h-screen overflow-hidden font-sans transition-colors duration-500 ${bg}`}>
      ...
      <motion.aside
        ...
        className={`relative border-r shrink-0 overflow-hidden flex flex-col z-20 ${sb}`}
      >
        <div className="p-5 flex flex-col h-full min-w-[290px]">
          ...
          <div className="mt-4 pt-4 border-t border-slate-800/30 space-y-1">
            ...
            <button className="flex items-center gap-3 w-full p-3 rounded-2xl hover:bg-slate-500/10 text-xs font-bold transition-colors opacity-50">
        ...
        <header className={`h-[72px] border-b flex items-center justify-between px-8 z-10 backdrop-blur-xl ${hdr}`}>
          <div className="flex items-center gap-5">
            <button
              ...
              className={`p-2.5 rounded-xl border transition-all ${isDark ? "border-slate-800 hover:bg-slate-800" : "border-slate-200 hover:bg-slate-100"}`}
            >
          ...
          <div className="flex items-center gap-3">
            <button className={`p-2.5 rounded-xl transition-all ${isDark ? "text-slate-500 hover:text-white hover:bg-slate-800" : "text-slate-400 hover:text-slate-900 hover:bg-slate-100"}`}>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar px-6 md:px-12">
          <div className="max-w-3xl mx-auto min-h-full flex flex-col">
            {messages.length === 0 ? (
              ...
              >
                  ...
                  <div className="max-w-lg space-y-3">
                    ...
                    <p className={`text-sm leading-relaxed font-medium ${isDark ? "text-slate-400" : "text-slate-500"}`}>
                      Axiom AI is a rule-based conversational engine powered by NLP, Flask, and MongoDB.
              ...
              /* ── Messages ── */
              <div className="space-y-10 py-10 animate-in fade-in duration-300">
